// Andrew Jun
// ajun@csu.fullerton.edu
// @SilentOne1
// Partners: @DerekCastanon
...
int main(int argc, char* argv[]) {
  ...
  double sum{0.0};
  bool first = true;
  for (std::string const &number : arguments) {
    if (first) {
      first = false;
    } else {
      sum += std::stod(number);
  ...
  double average{sum / (std::stoi(arguments.size()) - 1)}; // arguments.size() returns unsigned long
